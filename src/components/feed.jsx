...
export default function Feed() {
  ...
  const getTiers = useCallback(async () => {
    ...
    let islandNames = tierList
      .map((x) => x.substring(67, x.length - 3))
  ...
  const decryptPersistentPosts = async (e) => {
    ...
    for (let i = 0; i < encryptedPosts.length; i++) {
      try {
        ...
        let decryptedPost = CryptoJS.AES.decrypt(
          encryptedPosts[i].content.toString(),
          e.target.key.value
        ).toString();
    ...
    if (formType === 'cid') {
      for await (const buf of state.ipfs.cat(e.target.cid.value)) {
        try {
          ...
          let decryptedPost = CryptoJS.AES.decrypt(
            encryptedPost.substring(1, encryptedPost.length - 1),
            e.target.key.value
          ).toString();
          let post = JSON.parse(hex2a(decryptedPost));
      ...
      const [err0, res0] = await to(
        deroBridgeApi.daemon('get-transaction', {
    ...
    <FlexBoxColumn justify="center" className="">
      ...
      <form onSubmit={decryptPersistentPosts}>
        ...
        <FlexBoxRow className="pt-3" justify="even">
          ...
          <Button
            size="small"
            onClick={() => setFormType(formType === 'cid' ? 'txid' : 'cid')}
          >
            Toggle Form
          </Button>
        </FlexBoxRow>

  ...
  let tierList = Object.keys(scData)
    ...
    .map((key) => (
      <Subscribe
        ...
        perks={
          island.tiers[key.substring(key.length - 4, key.length - 3)].perks
        }
        amount={scData[key.substring(0, key.length - 2) + 'Am']}
        interval={scData[key.substring(0, key.length - 2) + 'I']}
        userAddress={state.userAddress}
        dba={state.deroBridgeApiRef}
        scid={state.scid}
        randomAddress={state.randomAddress}
        available={scData[key.substring(0, key.length - 2) + 'Av']}
      />
    ));
